...
computer_simulator<-function(x){
  ...
  t4=x4^2
  e<-rnorm(m,0,0.01)
  y=-sum(x1+x2+x3+x4)*exp(-sum(t1+t2+t3+t4))+e
  return(y)
}
computer_simulator<-function(x){
  m=nrow(x)
  ...
  t4=x4^2
  e<-rnorm(m,0,0.01)
  y=-20*exp(-sqrt((t1+t2+t3+t4)/4)/5)-exp(2*pi*(x1+x2+x3+x4)/4)+20+e
  return(y)
}
# 核心函数定义
R <- function(theta, h){
...
RA <- function(x, theta){
  ...
  for (i in 1:m) {
    for(j in 1:m){
      RA[i,j] = R(theta, c(U1a[i]-U1a[j], U2a[i]-U2a[j], U3a[i]-U3a[j], Va[i]-Va[j]))
    }
  }
  return(RA)
}
...
ra <- function(x, X, theta){
  ...
  ra <- matrix(0, m, n)
  for(i in 1:m){
    h <- cbind(U1-U1a[i], U2-U2a[i], U3-U3a[i], V-Va[i])
    for(j in 1:n){
      ra[i,j] <- R(theta, h[j,])
    }
  }
...
b <- function(x, X, theta){
  ...
  b_mat <- matrix(0, m, n)
  for (i in 1:n){
    b_mat[,i] <- P %*% c(1, U1[i], U2[i], U3[i], V[i]) + Q %*% rA[,i]
  }
  return(b_mat)
}
...
nystrom_features <- function(X, Xm, theta){
  ...
  if(sum(idx) == 0) {
    # 如果没有足够大的特征值，使用所有特征值
    idx <- rep(TRUE, length(eig$values))
  }
  
  U <- eig$vectors[, idx, drop = FALSE]
  Dinv <- diag(1 / sqrt(eig$values[idx] + 1e-10))
  
  # 检查维度
  if(ncol(U) == 0) {
    # 如果U为空，返回原始特征
    return(list(Phi = K_nm, Xm = Xm))
  }
...
fit_nystrom_MLE <- function(X, y, m){
  ...
  if(opt$convergence != 0 && opt$convergence != 1) {
    message("警告: 优化未收敛, 代码 = ", opt$convergence)
  }
...
for(ll_idx in 1:length(ll_values)){
  ...
  for(m_idx in 1:length(m_values)){
    ...
    for(t in 1:2){
      ...
      fit <- tryCatch({
        fit_nystrom_MLE(Xtr, ytr, m = m_current)
      }, error = function(e) {
        message("Nystrom拟合失败: ", e$message)
        return(NULL)
      })
      
      if(!is.null(fit)) {
        yhat <- tryCatch({
          predict_nystrom(fit, Xtr, ytr, Xte)
        }, error = function(e) {
          message("Nystrom预测失败: ", e$message)
          return(rep(mean(ytr), length(yte)))
        })
      } else {
    ...
    for(t in 1:2){
      ...
      fit <- tryCatch({
        fit_rff_MLE(Xtr, ytr, D = m_current)
      }, error = function(e) {
        message("RFF拟合失败: ", e$message)
        return(NULL)
      })
      
      if(!is.null(fit) && fit$convergence >= 0) {
        yhat <- tryCatch({
          predict_rff(fit, Xtr, ytr, Xte)
        }, error = function(e) {
          message("RFF预测失败: ", e$message)
          return(rep(mean(ytr), length(yte)))
        })
      } else {
        yhat <- rep(mean(ytr), length(yte))
