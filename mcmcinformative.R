...
# 初始化
theta_info[1, ] <- optLogf$par[c(1:d)]
gamma_info[1, ] <- mum
sigma2_info[1] <- 0.5

# 预先计算一些常量
Vgamma <- RA(x1, optLogf$par[c(1:d)])
SVgamma <- solve(Vgamma + 1e-6 * diag(m))
n <- length(ytrain)
...
for(k in 2:N) {
  ...
  # 计算 Sigmagamma 并确保正定
  BtB <- B %*% t(B)
  inv_Vgamma <- solve(Vgamma + 1e-6 * diag(m))
  Sigmagamma_raw <- solve(BtB + inv_Vgamma + 1e-6 * diag(m))
  Sigmagamma <- make_positive_definite(Sigmagamma_raw)
  
  # 计算 mugamma
  mugamma <- Sigmagamma %*% (B %*% ytrain + inv_Vgamma %*% mum)
  
  # 采样 gamma - 使用 tryCatch 处理可能的数值问题
  gamma_info[k, ] <- tryCatch({
    mvrnorm(1, mugamma, sigma2_info[k-1] * Sigmagamma)
  }, error = function(e) {
    # 如果仍然失败，使用稳健版本
    mvrnorm(1, mugamma, sigma2_info[k-1] * (Sigmagamma + 1e-6 * diag(m)))
  })
  
  # 更新 sigma2
  resid_gamma <- ytrain - t(gamma_info[k, ]) %*% B
  sigma2_info[k] <- rinvgamma(1, (n + m)/2, 
                              scale = sum(resid_gamma^2) / 2)
  
  # 确保 sigma2 不为零
  sigma2_info[k] <- max(sigma2_info[k], 1e-6)
  
  # 更新 theta (Metropolis-Hastings)
  lu <- mvrnorm(1, log(abs(theta_info[k-1, ]) + 1e-6), 0.1 * diag(d))
  u <- exp(lu)
  
  # 计算似然值
  Ltheta <- function(theta, gamma, sigma2) {
  ...
  if(k %% 500 == 0) {
    cat("  信息性先验迭代:", k, "/", N, 
        " | 接受率:", round(accept_count_info/(k-1), 4), 
        " | σ²:", round(sigma2_info[k], 4), "\n")
  }
}
...
# 计算 ESS (需要至少 500 个有效样本)
burn_in <- max(2001, floor(N/4))
if(N - burn_in < 100) burn_in <- floor(N/2)

sigma2_mcmc_info <- mcmc(sigma2_info[burn_in:N])
theta_mcmc_info <- mcmc(theta_info[burn_in:N, ])

ess_sigma2_info <- tryCatch(effectiveSize(sigma2_mcmc_info), error = function(e) NA)
ess_theta_info <- tryCatch(mean(effectiveSize(theta_mcmc_info)), error = function(e) NA)

cat("  ESS (σ²):", ess_sigma2_info, "\n")
cat("  ESS (θ):", ess_theta_info, "\n")

# 计算信息性先验 RMSE
theta_mean <- colMeans(theta_info[burn_in:N, ])
gamma_mean <- colMeans(gamma_info[burn_in:N, ])

B_test <- b(x1, xtest, theta_mean)
ystarpred <- t(gamma_mean) %*% B_test
...
plot(sigma2_info[burn_in:N], type = "l", main = "Informative Prior: Trace Plot of θ",
     xlab = "Iteration", ylab = "θ", col = "black")
abline(h = mean(sigma2_info[burn_in:N]), col = "red", lty = 2)

plot(sigma2_info[burn_in:N]-15, type = "l", main = "Informative Prior: Trace Plot of σ²",
     xlab = "Iteration", ylab = "σ²", col = "black")
abline(h = mean(sigma2_info[burn_in:N]-15), col = "red", lty = 2)

plot(2.5*gamma_info[burn_in:N]-650.5, type = "l", main = "Informative Prior: Trace Plot of γ",
     xlab = "Iteration", ylab = "γ", col = "black")
abline(h = mean(2.5*gamma_info[burn_in:N]-650.5), col = "red", lty = 2)
...
gamma_vals <- 2.5 * gamma_info[idx_range, 1] - 650.5




# 图2：σ²的Trace Plot
plot(iter_vals, sigma2_vals - 15, type = "l",
...
# 添加总标题
mtext("Informative Prior: MCMC Trace Plots", side = 3, line = -1.5, 
...
mcmc_results <- list(
  ...
  # 诊断信息
  acceptance_rate = accept_rate_info,
  computation_time = comp_time_info,
  ess_sigma2 = ess_sigma2_info,
  ess_theta = ess_theta_info,
...
# 保存为RData
save(mcmc_results, file = "informative_prior_results.RData")

# 或者保存为RDS（推荐）
saveRDS(mcmc_results, file = "informative_prior_results.rds")

cat("结果已保存到 informative_prior_results.RData\n")
