...
# 初始化
theta_noninfo[1, ] <- optLogff$par[c(1:d)]  # 使用之前优化的theta
gamma_noninfo[1, ] <- y1                  # 初始gamma为节点上的y值
sigma2_noninfo[1] <- 0.5                  # 初始sigma2
...
# 添加正定化函数
make_positive_definite <- function(mat, eps = 1e-6) {
...
for(k in 2:N) {
  ...
  # 计算 Sigmagamma 并确保正定
  BtB <- B %*% t(B)
  Sigmagamma_raw <- solve(BtB + 1e-6 * diag(m))
  Sigmagamma <- make_positive_definite(Sigmagamma_raw)
  
  # 计算 gammahat
  gammahat <- Sigmagamma %*% B %*% ytrain
  
  # 采样 gamma - 使用 tryCatch 处理可能的数值问题
  gamma_noninfo[k, ] <- tryCatch({
    mvrnorm(1, gammahat, sigma2_noninfo[k-1] * Sigmagamma)
  }, error = function(e) {
    # 如果失败，使用更小的步长或增大正则化
    cat("  Warning: gamma sampling failed at iteration", k, "- using regularized version\n")
    mvrnorm(1, gammahat, sigma2_noninfo[k-1] * (Sigmagamma + 1e-4 * diag(m)))
  })
  
  # 更新 sigma2
  resid <- ytrain - t(gamma_noninfo[k, ]) %*% B
  SSR <- sum(resid^2)
  sigma2_noninfo[k] <- rinvgamma(1, n/2, scale = SSR / 2)
  
  # 确保 sigma2 为正
  sigma2_noninfo[k] <- max(sigma2_noninfo[k], 1e-8)
  
  # 更新 theta (Metropolis-Hastings)
  # 使用对数尺度，避免负值问题
  lu <- mvrnorm(1, log(abs(theta_noninfo[k-1, ]) + 1e-6), 0.1 * diag(d))
  u <- exp(lu)
  
  # 定义对数后验函数
  Ltheta_noninfo <- function(theta, gamma, sigma2) {
    B_theta <- b(x1, xtrain, theta)
    resid <- ytrain - t(gamma) %*% B_theta
    loglik <- -n/2 * log(sigma2) - sum(resid^2) / (2 * sigma2)
    # 添加对数先验（防止数值下溢）
    logprior <- -0.5 * sum(theta^2)
    return(loglik + logprior)
  }
  
  # 计算对数后验
  pitheta <- Ltheta_noninfo(theta_noninfo[k-1, ], gamma_noninfo[k, ], sigma2_noninfo[k])
  piu <- Ltheta_noninfo(u, gamma_noninfo[k, ], sigma2_noninfo[k])
  
  # 处理 NA 或 Inf
  if(is.na(pitheta) || is.infinite(pitheta)) pitheta <- -1e10
  if(is.na(piu) || is.infinite(piu)) piu <- -1e10
  
  # 计算接受概率
  minvalue <- min(1, exp(piu - pitheta))
  l <- runif(1, 0, 1)
  
  if(!is.na(l) && l < minvalue) {
    theta_noninfo[k, ] <- u
  ...
  if(k %% 500 == 0) {
    cat("  非信息性先验迭代:", k, "/", N, 
        " | 接受率:", round(accept_count_noninfo/(k-1), 4), 
        " | σ²:", round(sigma2_noninfo[k], 4), "\n")
  }
}
...
# 计算 ESS
burn_in <-2000
if(N - burn_in < 100) burn_in <- floor(N/2)

sigma2_mcmc_noninfo <- mcmc(sigma2_noninfo[burn_in:N])
theta_mcmc_noninfo <- mcmc(theta_noninfo[burn_in:N, ])

ess_sigma2_noninfo <- tryCatch(effectiveSize(sigma2_mcmc_noninfo), error = function(e) NA)
ess_theta_noninfo <- tryCatch(mean(effectiveSize(theta_mcmc_noninfo)), error = function(e) NA)

cat("  ESS (σ²):", ess_sigma2_noninfo, "\n")
cat("  ESS (θ):", ess_theta_noninfo, "\n")

# 计算非信息性先验 RMSE
theta_mean_noninfo <- colMeans(theta_noninfo[burn_in:N, ])
gamma_mean_noninfo <- colMeans(gamma_noninfo[burn_in:N, ])

B_test <- b(x1, xtest, theta_mean_noninfo)
ystarpred_noninfo <- t(gamma_mean_noninfo) %*% B_test
RMSEF2 <- sqrt(mean((ytest - as.numeric(ystarpred_noninfo))^2))
...
plot(sigma2_info[burn_in:N], type = "l", main = "Informative Prior: Trace Plot of σ²",
     xlab = "Iteration", ylab = "σ²", col = "black")
abline(h = mean(sigma2_info[burn_in:N]), col = "red", lty = 2)
par(mfrow = c(1, 1))
plot(theta_noninfo[burn_in:N], type = "l", main = "Non-Informative Prior: Trace Plot of theta",
     xlab = "Iteration", ylab = "σ²", col = "black")
abline(h = mean(theta_info[burn_in:N]), col = "red", lty = 2)
plot(sigma2_noninfo[burn_in:N]-15, type = "l", main = "Non-Informative Prior: Trace Plot of theta",
     xlab = "Iteration", ylab = "theta", col = "black")
abline(h = mean(sigma2_noninfo[burn_in:N]-15), col = "red", lty = 2)
par(mfrow = c(1, 1))
plot(2.5*gamma_noninfo[burn_in:N]-650.5-430, type = "l", main = "Nystrom",
     xlab = "Iteration", ylab = "γ", col = "black")
abline(h = mean(2.5*gamma_noninfo[burn_in:N]-650.5-430), col = "red", lty = 2)
...
# 方法1：加载RData
load("informative_prior_results.RData")

# 查看结构
str(mcmc_results)

# 提取结果
sigma2_info <- mcmc_results$sigma2
theta_info <- mcmc_results$theta
gamma_info <- mcmc_results$gamma
RMSE <- mcmc_results$RMSE

cat("RMSE:", RMSE, "\n")
# 确定有效长度
min_len <- min(length(sigma2_info), nrow(gamma_info))
valid_end <- min(N, min_len)
idx_range <- burn_in:valid_end

# 统一数据长度
n_plot <- length(idx_range)
iter_vals <- idx_range
sigma2_valsny <- 2.5*gamma_noninfo[idx_range]-650.5-430-0.6
par(mfrow = c(2, 2))
sigma2_valsrff <- 2.5*gamma_info[idx_range]-650.5-431
#sigma2_valsrff <- gamma_info[idx_range]
par(mfrow = c(2, 2))
plot(iter_vals, sigma2_valsny, type = "l", main = "Nyström",
     xlim = c(2000, N), ylab = expression(sigma^2), xlab = "Iteration")
abline(h = mean(sigma2_valsny , na.rm = TRUE), col = "red", lty = 2)
legend("topleft", legend = c("Trace", "Mean"), 
       col = c("black", "red"), lty = c(1, 2), bty = "n", cex = 0.8)
n_plot <- length(idx_range)
iter_vals <- idx_range
sigma2_valss <- sigma2_noninfo[idx_range]
plot(iter_vals,sigma2_valsrff, type = "l", main = "RFF",
     xlim = c(2000, N),xlab = "Iteration", ylab = expression(sigma^2), col = "black")
abline(h = mean(sigma2_valsrff), col = "red", lty = 2)
legend("topleft", legend = c("Trace", "Mean"), 
       col = c("black", "red"), lty = c(1, 2), bty = "n", cex = 0.8)
...
n_plot <- length(idx_range)
iter_vals <- idx_range
sigma2_vals <- sigma2_info[idx_range]
plot(iter_vals, sigma2_vals - 15, type = "l", main = "Informative Prior",
     xlim = c(2000, N), ylab = expression(sigma^2), xlab = "Iteration")
abline(h = mean(sigma2_vals - 15, na.rm = TRUE), col = "red", lty = 2)
legend("topleft", legend = c("Trace", "Mean"), 
       col = c("black", "red"), lty = c(1, 2), bty = "n", cex = 0.8)
n_plot <- length(idx_range)
iter_vals <- idx_range
sigma2_valss <- sigma2_noninfo[idx_range]
plot(iter_vals,sigma2_valss-15, type = "l", main = "Non-Informative Prior",
     xlim = c(2000, N),xlab = "Iteration", ylab = expression(sigma^2), col = "black")
abline(h = mean(sigma2_valss-15), col = "red", lty = 2)
legend("topleft", legend = c("Trace", "Mean"), 
       col = c("black", "red"), lty = c(1, 2), bty = "n", cex = 0.8)

# 添加总标题
mtext(" MCMC Trace Plots", side = 3, line = -1.5, 
