...
computer_simulator<-function(x){
  ...
  j10<-rep(10,m)
  e<-rnorm(m,0,0.01)
  y=j1*t1+j2*t2+j3*t3+j4*t4+j5*t5+j6*t6+j7*t7+j8*t8+j9*t9+j10*t10+e
  return(y)
}
computer_simulator<-function(x){
  m=nrow(x)
  ...
  x10=x[,10]
  e<-rnorm(m,0,0.01)
  y=sin(8*x1+x2+x3+x4+x5+x6+x7+x8+x9+x10)+e
  return(y)
}
#simulation function
computer_simulator<-function(x){
  ...
  t10=x10^2
  e<-rnorm(m,0,0.01)
  y=-sum(x1+x2+x3+x4+x5+x6+x7+x8+x9+x10)*exp(-sum(t1+t2+t3+t4+t5+t6+t7+t8+t9+t10))+e
  return(y)
}
computer_simulator<-function(x){
  m=nrow(x)
  ...
  t10=(x10)^2
  e<-rnorm(m,0,0.01)
  y=-20*exp(-sqrt((t1+t2+t3+t4+t5+t6+t7+t8+t9+t10)/10)/5)-exp(2*pi*(x1+x2+x3+x4+x5+x6+x7+x8+x9+x10)/10)+20+exp(1)+e
  return(y)
}
R<-function(theta,h){
  RH<-sum(-h^2%*%theta)
...
RA<- function(x,theta){
  ...
  for (i in 1:m) {
    for(j in 1:m){
      RA[i,j]= R(theta,c(U1a[i]-U1a[j],U2a[i]-U2a[j],U3a[i]-U3a[j],U4a[i]-U4a[j],U5a[i]-U5a[j],U6a[i]-U6a[j],U7a[i]-U7a[j],U8a[i]-U8a[j],U9a[i]-U9a[j],Va[i]-Va[j]))
    }
  }
  return(RA)
}
ra <- function(x,X,theta){
  ...
  ra<-matrix(0,m,n)
  for(i in 1:m){
    h <- cbind(U1-U1a[i],U2-U2a[i],U3-U3a[i],U4-U4a[i],U5-U5a[i],U6-U6a[i],U7-U7a[i],U8-U8a[i],U9-U9a[i],V-Va[i])
    for(j in 1:n){
      ra[i,j]=R(theta,h[j,])
    }
  }
...
b<-function(x,X,theta){
  ...
  U9<-X[,9]
  V<-X[,10]
  GA<-cbind(rep(1,m),U1a,U2a,U3a,U4a,U5a,U6a,U7a,U8a,U9a,Va)
  I<-rep(1,m)
  RAT<-solve(RAA+1e-8*diag(m))
  GRA<-solve(t(GA)%*%RAT%*%GA+1e-8*diag(11))
  P<-RAT%*%GA%*%GRA
  Q<-(diag(I)-RAT%*%GA%*%GRA%*%t(GA))%*%RAT
  b <- matrix(0,m,n)
  for (i in 1:n){
    b[,i]<-P%*%c(1,U1[i],U2[i],U3[i],U4[i],U5[i],U6[i],U7[i],U8[i],U9[i],V[i])+Q%*%rA[,i]
  }
  return(b)
}
CA <- function(X, m){
...
for(ll_idx in 1:length(ll_values)){
  ...
  for(m_idx in 1:length(m_values)){
    ...
    for(t in 1:2){
      ...
      u6 <- runif(NT + ll_current, 0, 1)
      u7 <- runif(NT + ll_current, 0, 1)
      u8 <- runif(NT + ll_current, 0, 1)
      u9 <- runif(NT + ll_current, 0, 1)
      v <- runif(NT + ll_current, 0, 1)
      xx <- cbind(u1, u2, u3,u4,u5, u6, u7,u8,u9,v)
